    ...
    await mockNetworkImages(() async {
      ...
      ielALeQuizSuivant(
        const Quiz(
          ...
          question: question,
          responses: [
            QuizResponse(response: 'Manger moins de produits de saison', exact: false),
            QuizResponse(response: 'Diminuer la consommation de viande', exact: true),
            QuizResponse(response: 'Privilégier la volaille et le porc', exact: false),
            QuizResponse(response: 'Réduire ses déchets', exact: false),
          ],
          points: 5,
          explicationOk:
  ...
  testWidgets("La bonne réponse affiche l'explication ok", (final tester) async {
    ...
    await mockNetworkImages(() async {
      ...
      const quiz = Quiz(
        ...
        responses: [
          QuizResponse(response: 'Manger moins de produits de saison', exact: false),
          QuizResponse(response: reponse, exact: true),
          QuizResponse(response: 'Privilégier la volaille et le porc', exact: false),
          QuizResponse(response: 'Réduire ses déchets', exact: false),
        ],
        points: 5,
      ...
      verify(() => ScenarioContext().dioMock!.patch<dynamic>(Endpoints.quiz(quiz.id), data: {'pourcent': 100}));
    });
  });
  ...
  testWidgets("Les mauvaises réponses affiche l'explication ko", (final tester) async {
    ...
    await mockNetworkImages(() async {
      ...
      const quiz = Quiz(
        ...
        question: 'Quelle action est la plus efficace pour une alimentation plus durable ?',
        responses: [
          QuizResponse(response: reponse, exact: false),
          QuizResponse(response: 'Diminuer la consommation de viande', exact: true),
          QuizResponse(response: 'Privilégier la volaille et le porc', exact: false),
          QuizResponse(response: 'Réduire ses déchets', exact: false),
        ],
      ...
      await ielAppuieSur(tester, Localisation.valider);
      ielVoitLeTexte(Localisation.pourquoi);
      verify(() => ScenarioContext().dioMock!.patch<dynamic>(Endpoints.quiz(quiz.id), data: {'pourcent': 0}));
    });
  });
}
...
Future<void> _init(final WidgetTester tester, final String titre) async {
  ielEstConnecte();
  ScenarioContext().dioMock!.patchM(Endpoints.quiz('42'));
